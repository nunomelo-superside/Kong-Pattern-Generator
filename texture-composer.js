        ...
        for (let yy = 0; yy < height; yy += 1) {
          for (let xx = 0; xx < width; xx += 1) {
            ...
            if (state.mode === "paint") {
              ...
              if (nextInk <= eraseInkSnapThreshold && influence >= eraseSnapInfluenceFloor) {
                nextInk = 0;
              }
            }

            const next = (1 - Math.max(0, Math.min(1, nextInk))) * 255;
      ...
      function buildLayerSvg(layerName) {
        ...
        for (let y = spacing / 2; y < height; y += spacing) {
          ...
          for (let x = spacing / 2 + rowOffset; x < width; x += spacing) {
            ...
            const rawRadius = curvedDarkness * maxRadius;
            if (rawRadius <= 0.01) {
              continue;
            }
            if (rawRadius < effectiveSizeCutoff) {
              continue;
            }
            const radius = Math.max(effectiveMinDot, rawRadius);
            const arm = radius;
      ...
      generatorModeSelect.addEventListener("change", async () => {
        ...
        window.location.href = nextMode === "image" ? "./pattern-generator.html" : "./index.html";
      });
